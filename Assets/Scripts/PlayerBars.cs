...
public class PlayerBars : MonoBehaviour {
	...
	void Update () {
        ...
        if (xpbar == true)
        {
            ...
            xp2 = sm.xp;
            level = sm.playerlvl -1;
            text.content = sm.playerlvl.ToString();
            xp3 = xp2 / levelcaps[level];
            healthPercent = 100 - xp3 * 100;
            if (xp2 > levelcaps[level])
            {
                sm.xp = sm.xp - levelcaps[level];
                sm.playerlvl++;
                sm.levelup();
            }
        }
